...
result=foreach(i=1:reps,.packages=c('combinat','mrfDepth','nloptr',"optimx"),
               .combine = "cbind",.verbose = T,.errorhandling="remove") %dopar% {
                 ...
                 secondattempt=MVQBAFD_fit(data = X,basefunc = basefunc,seed = seed+i,maxiter = maxiter,tol = tol,numstarts=numstarts)
                 
                 
                 fits=c(as.matrix(secondattempt$`fitted parameters`,nrow=1),secondattempt$`log likelihood fit`)
...
# please select the correct data and change the directories where output is written
...
result=foreach(i=1:nmods,.packages=c('nloptr','optimx'),
               .combine = "cbind",.verbose = T,.errorhandling="remove") %dopar% {
                 ...
                 filename=paste0("~/pokemon/M",i,".csv")
                 write.csv(output,file=filename)
               }
stopCluster(cl)
x11()
...
for (i in 1:nmods){
  ...
  temp_data = t(read.csv(paste0("~/pokemon/M",i,".csv"), row.names=1))
...
save(modelfit,file="~/pokemon/fittedmodels_pokemon.Rdata")
